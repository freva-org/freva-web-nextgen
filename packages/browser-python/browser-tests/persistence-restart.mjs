...
/** Thrown to leave the OPFS half once the worker has said there is no workspace to test. */
...
let status = "pass";
let detail = "";
/** Set when the worker in this engine has no disk-backed workspace: the OPFS half cannot exist. */
let unavailable;
try {
  ...
  /** One whole browser lifetime against the same on-disk profile, in the selected engine. */
  ...
  const first = await launch(async (page) => {
    ...
    const workspace = await page.evaluate(() => window.__py.engine.workspace);
    // Asked of the worker, in this engine. Without a disk-backed workspace there are no session
    // directories to outlive anything; the fallback is checked and the IndexedDB half still runs.
    const fallback =
      workspace?.available === true ? null : await workspaceFallbackChecks(page, workspace);
    const state = await page.evaluate(async (hasWorkspace) => {
      if (!hasWorkspace) return { session: null, artifacts: [] };
    ...
    return { ...state, sessions, fallback };
  });

  if (first.fallback) {
    checks.push(...first.fallback.checks);
    unavailable = first.fallback.reason;
    // The credential half needs no workspace: it is the browser's own IndexedDB, across a restart.
    const credential = await launch((page) =>
      page.evaluate(
        () =>
    ...
    ok(
      ...
      credential === "a token, as far as this test is concerned",
  ...
  if (error instanceof WorkspaceUnavailable) {
    // Not a failure and not a pass: `unavailable` carries the reason to `report`.
  } else {
...
if (unavailable && status === "pass") status = "not-applicable";

process.exit(
  report(`persistence across a real browser restart (${ENGINE})`, {
    status,
    ...(detail ? { detail } : {}),
